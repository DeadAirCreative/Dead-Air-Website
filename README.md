...
- **Scroll-expansion hero** (`index.html`): the homepage opens on a small centred card autoplaying a muted, looping cut of the Nevuary Jacket 1 reel; scrolling through the tall `#hero-track` section (240vh) grows the card into a near-full-height frame while the split headline slides off both edges and the background photo fades out — then the subhead + CTAs fade in and the page continues normally. Implementation notes worth knowing before touching it:
  ...
  - The media frame keeps the reel's **9:16 portrait aspect** as it expands (capped at 82vh / 92vw) instead of stretching to a widescreen letterbox — cropping a vertical reel to landscape would discard most of the frame and upscale what's left to mush.
  ...
  - `images/portfolio/video/home-hero.mp4` is a **14s cut (5s–19s) of the source reel** encoded with `avconvert -p PresetAppleM4V720pHD` (404×720 H.264, 5.6MB — the full 26s at the plain 720/1080 presets came out 22–30MB, far too heavy for a hero). `images/portfolio/home-hero-poster.webp` is the cut's first frame, so poster→playback is seamless. Re-run that encode (see git history of `scripts/process-videos.sh` style) if the reel ever changes.
