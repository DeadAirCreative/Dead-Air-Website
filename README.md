...
`process-galleries.sh` clears each project's destination folder before renumbering, so a shrunk source project doesn't leave stale higher-numbered files behind. If a project's source folder is deleted entirely (as happened with Landscape), also remove its entry from `PROJECTS` in `generate-manifest.py`, delete its grid thumbnail (`images/portfolio/<slug>.webp`), and remove its card from `portfolio.html`.

Grid thumbnails for the 4 video-tagged projects (Nevuary, Fresh Pop, Flawless Finesse Detailing, NI Hospice) autoplay a short muted loop (`images/portfolio/video/*-preview.mp4`, ~4-5MB each at 720p, only plays while scrolled into view — pauses off-screen, respects `prefers-reduced-motion`).

### Media compression
`images/` is now **~530MB**, up from ~124MB — see the video note below for why, and use [Git LFS](https://git-lfs.com) for `images/portfolio/full/` and `images/portfolio/video/` if repo size becomes a problem for your host.
...
- **Video**: every clip (full-gallery, grid previews, and the hero — see below) is now encoded with **fixed bounding-box presets** (`Preset1280x720` for gallery/preview clips, `Preset1920x1080` for the hero), not one of avconvert's adaptive "quality" presets. This replaced an earlier approach using `PresetMediumQuality`, which looked great on one test clip (full native resolution, smaller file) but turned out to pick output resolution **adaptively per source bitrate** — it silently downscaled the highest-bitrate footage (the hero's 20.5Mbps source) to under 570px wide, actually worse than what was already live, while leaving a lower-bitrate clip at full 1080p. Confirmed via a controlled test: `Preset1280x720` gave identical, predictable 720×1280 output on both a modest-bitrate clip and the highest-bitrate 4K source in the set. The fixed-resolution tradeoff is a much bigger `images/portfolio/video/full/` (~409MB, up from ~108MB) for a real, verified quality jump — frame-by-frame comparison at the old vs new encode showed jacket text and background detail that was previously mush is now legible. `scripts/process-videos.sh` is the authoritative regen path if source footage ever changes; the 4 grid previews were re-cut by hand from the same source clips (no script) since there are only 4 and they're short trims, not full processing runs.
...
- **Scroll-expansion hero** (`index.html`): the homepage opens on a small centred card autoplaying a muted, looping cut of the Nevuary Jacket 1 reel; scrolling through the tall `#hero-track` section (240vh) grows the card into a near-full-height frame while the headline **builds in word by word** (eyebrow first, then each word fades/rises on its own slice of scroll progress — `revealStart`/`revealSpan`/`wordWindow` in `js/main.js`) and the background photo fades out. The headline ends fully settled over the expanded reel — the scrim deepens slightly (0.25→0.4) as the text lands so white type stays legible over bright frames — then the subhead + CTAs fade in and the page continues normally. The h1 keeps its full sentence as an `aria-label` (word spans are `aria-hidden`), so screen readers hear one clean phrase. Implementation notes worth knowing before touching it:
  ...
  - `images/portfolio/video/home-hero.mp4` is the **full 28s "DA Hero Vid 2" showreel** (source in `Images:Video/`, 1920×1080 @ 133MB) encoded with `avconvert -p Preset1920x1080` — full native resolution, 33MB. (Originally "DA Hero Vid 1", a 20s cut at 49MB source / 23MB encoded — swapped out once Vid 2 was supplied.) Originally shipped at `Preset960x540` (10MB) to save weight, but the hero's max on-screen size never exceeds ~1600×900 (see the sizing constants below), so 540p was always being upscaled ~1.67x and visibly soft — this was the main complaint that prompted the site-wide video quality pass. At 1080p the hero is never upscaled at any viewport. `images/portfolio/home-hero-poster.webp` is the reel's first frame (regenerated via `qlmanage -t` + `cwebp -q 82` when the video changes, since this repo has no `ffmpeg`), so poster→playback is seamless. The video and poster URLs in `index.html` carry a `?v=` cache-bust param: **bump it whenever either file changes** or returning visitors keep the old video. Re-encode with `Preset1920x1080` if the reel is ever replaced.
